...
def args_dict_to_dataclass(eval_args: dict[str, object], config_dataclass, save_config: bool = True) -> tuple[Any, str]:
    """eval_args : arguments passed to create the eval config. `CONFIG_FILE_KEY` is a reserved name to load a set of
    parameters from a config file. config_dataclass: a dataclass used to define the config arguments, types and
    default values save_config : whether to save the config in a file named `EVAL_CONFIG_FNAME` in the output_dir.
    """
...
def cli_parser(argv: list[str]) -> tuple[dict[str, Any]]:
    """A method to parse argv and output a dict of eval arguments, and model building arguments. - `argv` can come from
    the command line directly, or from a subset of the command line arguments, as in dinov3.run.submitit -
